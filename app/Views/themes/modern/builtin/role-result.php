			...
			foreach ($result as $key => $val) {
						...
						if (key_exists($val['id_role'], $module_allowed)) {
							if (!key_exists($val['id_module'], $module_allowed[$val['id_role']])) {
								echo '<p class="mt-0 mb-0">
											<span class="text-danger">Module <strong>' . $module_list[$val['id_module']]['nama_module'] . '</strong> belum di assing ke role ini, silakan <a href="'. $config->baseURL .'module-role" target="_blank">assign</a> terlebih dahulu</span>
									</p>';
							}
						} else {
							echo '<p class="mt-0 mb-0">
				...
				echo '</td>
						...
						<td>
							...
							<a target="_blank" href="'.current_url().'/edit?id='. $val['id_role'] .'" class="btn btn-success btn-xs mr-1"><i class="fa fa-edit"></i>&nbsp;Edit</a>
